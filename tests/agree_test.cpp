...
TEST(agree_closed_range_at_the_maximum) {
    CHECK(agrees("pub func answer() -> i64:\n    var n: i64 = 0\n    for i: u8 in 250..=255:\n        n += 1\n    for j: i8 in 125..=127:\n        n += 1\n    outer: for k: u16 in 65534..=65535:\n        for m: u8 in 255..=255:\n            n += 1\n            continue outer\n    return n + 29\n"));
}
...
// (Ãƒâ€šÃ‚§14.4): to standard output from `print`, into the caller's buffer from `format`.
...
// `f16` is a real binary16 (Ãƒâ€šÃ‚§5.1): two bytes in memory, every result rounded to half precision,
...
TEST(agree_cast_gives_its_operand_no_context) {
    ...
    // literal is read in the cast's type; `-128` is an `i8` (Ãƒâ€šÃ‚§4.2, Ãƒâ€šÃ‚§7.5)
