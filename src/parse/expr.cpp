//==============================================================================================
//
//   parse/expr - Expressions
//
//   DESCRIPTION:
//       Layered precedence for base.md §7.10, casts against calls by declaration, `else` and
//       `catch` as suffixes, formatted strings, allocation forms, lambdas, and the
//       disambiguation of `[` between indexing, generic arguments, and array types.
//
//==============================================================================================
    ...
    if (at(TokenKind::KwReturn) || at(TokenKind::KwBreak) || at(TokenKind::KwContinue)) {
        ...
        if (t.kind == TokenKind::KwReturn) {
            ...
            if (!at(TokenKind::Newline) && !at(TokenKind::Dedent) && !at(TokenKind::EndOfFile) &&
                !at(TokenKind::RParen) && !at(TokenKind::Comma) && !at(TokenKind::RBracket)) {
