//==============================================================================================
//
//   emit/stmt - Statements, scopes, and cleanup in C
//
//   DESCRIPTION:
//       Bindings, assignment, branches, loops with labels, `match` as a switch, `return`
//       through the deferred calls of every enclosing scope, `defer` and `errdefer`
//       registration, `with` allocator scopes, and `free` (base.md §6, §8, §12).
//
//==============================================================================================
            ...
            if (is_span(n->ty) && is_array(st)) {
                ...
                init = "((lb_span){" + init + ".d, " + nbuf + "})";
                   ...
                   (n->ty->kind == TypeKind::Struct || n->ty->kind == TypeKind::Union ||
                    ...
                    n->ty->kind == TypeKind::Allocator || n->ty->kind == TypeKind::Interface ||
                    is_tup(n->ty))) {
            init = "{0}";
    ...
    if (is_int(st) || is_int_enum(st) || (st != nullptr && st->kind == TypeKind::Bool)) {
        ...
        for (Node* arm = n->body; arm != nullptr; arm = arm->next) {
            for (Node* pat = arm->left; pat != nullptr; pat = pat->next) {
                ...
                } else if (pat->resolved != nullptr && pat->resolved->kind == NodeKind::EnumCase) {
                    line("case " + std::to_string(emit_case_int(st->decl, pat->resolved)) + "LL:");
