...
// `path:line: assert failed: condition`: what a failed `assert` reports (Ã‚Â§11.6).
auto Emitter::assert_message(Node* call) -> string {
    // the position is the trap's (Ã‚Â§11.5, Ã‚Â§11.6)
...
auto Emitter::produces_opt(Node* n) -> bool {
    ...
    if (n->kind == NodeKind::Catch) {
        return true; // a `catch` typed `T?` builds the optional itself (Ã‚Â§11.4)
    }
...
auto Emitter::emit_literal(Node* n) -> string {
    ...
    if (n->op == TokenKind::FloatLit) {
        ...
        // the bits the literal rounds to at its own width (base.md Ã‚Â§4.3), as C's exact
...
// `a == b` for values of type `t`, component by component (Ã‚Â§7.4); `a` and `b`
...
auto Emitter::emit_binary(Node* n) -> string {
    ...
    if (!at_file_scope && n->op != TokenKind::KwAnd && n->op != TokenKind::KwOr &&
        (may_have_effect(n->left) || may_have_effect(n->right))) {
        const string left = "_lb_bl" + std::to_string(tmp());
        const string right = "_lb_br" + std::to_string(tmp());
        const string prefix = "__auto_type " + left + " __attribute__((unused)) = " + L + "; __auto_type " + right + " __attribute__((unused)) = " + R + "; ";
        return sequenced(prefix, emit_binary_values(n, left, right));
    }
    return emit_binary_values(n, L, R);
}

auto Emitter::emit_binary_values(Node* n, const string& L, const string& R) -> string {
    TokenKind op = n->op;
...
// One scalar arithmetic, wrapping, saturating, bit, or shift operation of Ã‚Â§7.2 and Ã‚Â§7.3 on
...
auto Emitter::emit_arith(Type* t, TokenKind op, const string& L, const string& R) -> string {
    ...
    if (at_file_scope) {
        ...
        // evaluated without overflow (Ã‚Â§6.4); C wants the plain operator there
...
// Lane-wise arithmetic (Ã‚Â§5.12): the operands are bound once, each lane of the result is the
...
// `T[N](x)`: every lane is `x`, computed once (Ã‚Â§5.12); in a global's initialiser `x` is a
...
// `v.sum()`, `v.min()`, `v.max()`: the lanes folded in order (Ã‚Â§5.12); a float `min` or `max`
...
auto Emitter::emit_conv(Node* src, Type* dest, bool checked) -> string {
    ...
    if (dest->kind == TypeKind::CStr && st != nullptr && st->kind == TypeKind::Str) {
        return "lb_cstr_of(" + e + ")"; // raw address borrow; termination/lifetime belong to the caller (Ã‚Â§5.5)
    }
    if (is_int_enum(dest)) {
        if (checked) {
...
auto Emitter::emit_member(Node* n) -> string {
    ...
    if (ot != nullptr && ot->kind == TypeKind::Module) {
        ...
        // initialiser its value stands in for its name (Ã‚Â§6.4)
        if (at_file_scope && n->resolved != nullptr && n->resolved->kind == NodeKind::Const &&
            n->resolved->left != nullptr) {
            return "(" + emit_expr(n->resolved->left) + ")";
        }
...
auto Emitter::emit_index(Node* n, bool one_past) -> string {
    ...
    // under `&` the index may equal the length: C's one past the end (Ã‚Â§7.7)
...
auto Emitter::emit_array_lit(Node* n) -> string {
    if (n->ty != nullptr && n->ty->kind == TypeKind::Span) {
        // a literal where a span is wanted (Ã‚Â§5.4): an array of the function's, declared at
