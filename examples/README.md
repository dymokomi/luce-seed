...
| `lexer/` | A lexer for a Base subset written in Base: keyword tables as `str[N]` constants, layout with a fixed indent stack, `ErrorCode` constants shared across modules, `files.read` and `c.str` arguments, counts in an array indexed by an enum. |
